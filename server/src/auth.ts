...
export function signToken(user: AuthUser): string {
  return jwt.sign({ sub: user.id, pseudo: user.pseudo }, config.jwtSecret, {
    expiresIn: '30d',
  });
}
...
export function verifyToken(token: string): AuthUser | null {
  try {
    const payload = jwt.verify(token, config.jwtSecret) as { sub: string; pseudo: string };
...
authRouter.post('/login', async (req, res) => {
  ...
  const user: AuthUser = { id: row.id, pseudo: row.pseudo };
  res.json({ token: signToken(user), user });
});
